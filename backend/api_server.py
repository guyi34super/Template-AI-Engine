...
# Rust bridge
from core.rust_bridge import close_rust_client, rust_health
...
app = FastAPI(
    title="AI-RAG Document Processing Engine",
    description="Enterprise-grade platform for document extraction, intelligent data mapping, validation, and LLM-powered processing",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    openapi_tags=[
        {"name": "Authentication", "description": "User auth, JWT tokens, MFA"},
        {"name": "Templates", "description": "Document template CRUD with versioning"},
        {"name": "Document Extraction", "description": "Upload, extract, and classify documents"},
        {"name": "Validation", "description": "Regex validation rules and pattern testing"},
        {"name": "Mapping Engine", "description": "Field mapping between schemas"},
        {"name": "Chat Engine", "description": "Natural language data manipulation"},
        {"name": "PowerMemory", "description": "Cross-session AI memory"},
        {"name": "Export", "description": "Download and database push"},
    ]
)
...
# Security, compression & custom middleware  (order matters: outermost runs first)
app.add_middleware(SecurityHeadersMiddleware)
app.add_middleware(GZipMiddleware, minimum_size=500)
app.add_middleware(IdempotencyMiddleware)
app.add_middleware(RateLimitMiddleware, ip_limit=60, user_limit=120, window=60)
...
# Include Template Engine routes
from core.template_routes import router as template_router
app.include_router(template_router)

# Include Validation Engine routes
from core.validation_routes import router as validation_router
app.include_router(validation_router)

# Include Export routes
from core.export_routes import router as export_router
app.include_router(export_router)

# Include WebSocket routes
from core.ws_routes import router as ws_router
...
async def startup_event():
    """Initialize PowerMemory, Redis, DB, ChromaDB on startup"""
    global power_memory
    power_memory = get_power_memory()
    # Initialise Redis (graceful no-op when unavailable)
    await init_redis()
    # Initialise async database if DATABASE_URL is set
    from core.db import init_async_db, is_async_db
    if is_async_db():
        await init_async_db()
    # Initialise ChromaDB collections
    from core.chromadb_client import init_chromadb
    init_chromadb()
    # Check Rust service health
    try:
        rust_status = await rust_health()
        print(f"🦀 Rust data-tier connected: {rust_status.get('version', 'unknown')}")
    except Exception as e:
        print(f"⚠️  Rust data-tier unavailable: {e} — CPU-intensive ops will fall back to Python")
...
async def shutdown_event():
    """Finalize audit logs and close connections on shutdown"""
    try:
        print("\n🔄 Shutting down server...")
        await close_redis()
        from core.llm_client import close_llm_client
        await close_llm_client()
        from core.supermemory_client import close_supermemory
        await close_supermemory()
        await close_rust_client()
        from core.embedding_client import close_embedding_client
        await close_embedding_client()
        audit.finalize()
        print("✅ All connections closed, audit logs finalized")
    except Exception as e:
